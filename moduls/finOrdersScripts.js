...
if (user_name == '') window.location.href = 'index.html';
$('.LK_text').text(user_name);
$('#id_user_name').val(user_name);
$('#id_type_selection').val('week');
...
function GetMakeOrd() {
    $.ajax(
        {
            url: 'http://velomessenger.ru/wp-content/themes/velomessenger/mobApk/finOrdersMobApk.php',
            type: 'POST',
            dataType: 'html',
            async: false,
            data: $('#id_sorting_ord').serialize(),
    ...
    if (res[0].number_fields > 0) {
        ...
        showDate = curDate;
        if (curDate == TodayDate) showDate = 'сегодня';
        $('.fo_orders').append('<div class="day_divider">Доставлено: ' + showDate + '</div>');
        ...
        for (var i = 1; i <= res[0].number_fields; i++) {
            ...
            if (i == res[0].number_fields) lastOrderClass = ''; else if (res[i].date != res[i + 1].date) lastOrderClass = ' s_order_last'; else lastOrderClass = '';
        ...
        $('.final_summ').text('Заработок: ' + Math.round(0.7 * res[0].final_summ) + ' руб.');
    }
}
...
function ShowNewEntries(interval) {
    $('.final_summ').text('');
    $('#id_type_selection').val(interval);
    $('#id_show_all_href, #id_show_week_href, #id_show_month_href').removeClass('marked_href');
    $('#id_show_'+interval+'_href').addClass('marked_href');
...
$('#id_inp_search').change(function(){
    ShowNewEntries($('#id_type_selection').val());
});
...
$('body').delegate('.short_order','click',function(){
    var numb_order_value = $(this).children('.s_order_numb').text().slice(2, 10);
    storage.setItem('one_order_numb', numb_order_value);
    storage.setItem('type_page_from', 'finOrd');
    window.location.href = 'oneOrder.html';
});

$('.new_order_mess').click(function () {
    window.location.href = 'freeOrders.html';
});

$('#id_left_arrow').click(function () {
    window.location.href = 'currOrders.html';
});
