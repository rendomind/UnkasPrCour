var last_numb_ord=0, count_free_ord=0, free_order_exist=false;
...
function GetMakeOrd() {
    ...
    $.ajax(
        {
            url: 'http://velomessenger.ru/wp-content/themes/velomessenger/mobApk/freeOrdersMobApk.php',
            type: 'POST',
            dataType: 'html',
            async: false,
    ...
    if (res[0].number_fields > 0) {
        last_numb_ord = res[0].last_numb_ord;
        count_free_ord = res[0].count_free_ord;
...
setInterval(function () {
    ...
    if ((last_numb_ord != res_check.last_numb_ord) || (count_free_ord != res_check.count_free_ord)) GetMakeOrd();
}, 5000);
...
$('body').delegate('.short_order','click',function(){
    ...
    storage.setItem('type_page_from', 'freeOrd');
